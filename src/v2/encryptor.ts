 ...
 */
function encryptor(data: string | Buffer, key: string, encoding: Encoding = 'base64') {
  /**
 ...
 * @returns return encrypted string with selected encoding
 */
export function encrypt(data: string | Buffer, key: string, options: IEncryptionOptions = {}): string {
  if (key.length !== 32) {
 ...
 * @returns return encrypted string with selected encoding
 */
export function encryptFile(pathToFile: string, key: string, options: IEncryptionOptions = {}): string {
  if (key.length !== 32) {
    throw new InvalidKeyError('Key must be 32 characters')
  }
  const encryptionCount = options.encryptionCount || 1
